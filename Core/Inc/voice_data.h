 ...
 * 포맷: 22050 Hz, G.711 μ-law 8-bit, mono
 * 화자: ko-KR-InJoonNeural (edge-tts Neural)
 *
 * 인덱스:
...
#define VOICE_SAMPLE_RATE  22050u
...
typedef struct {
    const uint8_t *ulaw;         /* G.711 μ-law 바이트 스트림 */
    uint32_t       sample_count; /* 총 샘플 수 = 바이트 수 */
} VoiceClip;

extern const VoiceClip voice_clips[VOICE_NUM_CLIPS];
extern const int16_t   voice_ulaw_table[256];
